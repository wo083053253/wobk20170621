...
echo "[$(date +%Y%m%d-%H:%M:%S)] Submit create volume $vol_name request"
cinder create --display_name $vol_name --volume-type $vol_type $vol_size >/dev/null 
#cinder create --display_name $vol_name --volume-type CEPH_SSD $vol_size >/dev/null 
#cinder create --display_name $vol_name --volume-type CEPH_HYBRID $vol_size >/dev/null 
#cinder create --display_name $vol_name $vol_size >/dev/null
status=$?
if [ $status != 0 ]; then
    extra_info="create_volume_request_fail_directly"
	echo "[$(date +%Y%m%d-%H:%M:%S)] Create volume $vol_name request failed directly"
	log_result
	exit 1
fi
echo "[$(date +%Y%m%d-%H:%M:%S)] Create volume $vol_name request submitted"

#Wait for new volume to be available
echo "[$(date +%Y%m%d-%H:%M:%S)] Wait for volume $vol_name to get Available"
start_time=$(date +%s)
while :
do
	vol_status=$(cinder show $vol_name | awk '/status/&&/available/ {print $(NF-1)}')
	if [ ! -z "$vol_status" ]; then
		break
	fi							
	# If volume is not available before timeout, will not wait any more
	current_time=$(date +%s)
	if [ $((current_time - start_time)) -gt $vol_waitfor_available_timeout ]; then
	    extra_info="volume_not_available_before_timeout"
		echo "[$(date +%Y%m%d-%H:%M:%S)] Volume $vol_name failed to get Available before timeout"
		log_result
		exit 1
	fi
	sleep "$polling_interval"	
done
echo "[$(date +%Y%m%d-%H:%M:%S)] Volume $vol_name get Available successfully"	
