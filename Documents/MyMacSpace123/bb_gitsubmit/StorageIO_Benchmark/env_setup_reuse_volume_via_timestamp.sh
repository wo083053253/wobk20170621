...
do
	...
	read -u 6
	{
	./vm_provision_thread_reuse_volume.sh "$kvm_host" $i fio_vm_"$kvm_host"_"$i"_"$timestamp" "$network_name" $network_id $key_name $private_key_file $secgroup_name 
       #./vm_provision_thread_reuse_volume.sh "$kvm_host".blueboxgrid.com $i fio_vm_"$kvm_host"_"$i"_"$timestamp" "$network_name" $network_id $key_name $private_key_file $secgroup_name 
    echo >&6
    } & 
    done
done
