...
eventgen_rest_ip=`tail -n +2 hosts/eventgen_rest`
dashboard_proxy_dev=`cat group_vars/all | grep dashboard_proxy | awk '{print $2;}'`/dev/eventgen/$eventgen_rest_ip
...
message_hub_servers=`cat group_vars/all_eventgen | grep message_hub_servers | awk '{print $2;}'`
message_hub_token=`cat group_vars/all_eventgen | grep message_hub_token | awk '{print $2;}'`
