...
class LogStash::Outputs::PagerDuty2 < LogStash::Outputs::Base
  config_name "pagerduty2"
  milestone 1
  ...
  public
  def encodeDetails(event, details)
    out = Hash.new
    details.each do |key, value|
      @logger.debug("PD Details added:" , key => event.sprintf(value))
      if value.class == Hash
      	out["#{key}"] = encodeDetails(event, value)
      else
      	out["#{key}"] = event.sprintf(value)
      end
    end
    return out
  end
  ...
  def receive(event)
    ...
    @details.each do |key, value|
      ...
      if value.class == Hash then
      	pd_event[:details]["#{key}"] = encodeDetails(event, value)
      else
